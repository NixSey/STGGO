...
func fjson(str string) interface{} {                                           var a interface{}
        json.Unmarshal([]byte(str), &a)                                        return a
}                                                                      
func generateRandomNumber(numberOfDigits int) int {                            maxLimit := int64(int(math.Pow10(numberOfDigits)) - 1)
        lowLimit := int(math.Pow10(numberOfDigits - 1))                        randomNumber, _ := rand.Int(rand.Reader, big.NewInt(maxLimit))
        randomNumberInt := int(randomNumber.Int64())                   //      if randomNumberInt < 1 {
...
func pr(text string) {
        if len(text) > 0 || text != "" {
                ...
                if !strings.HasPrefix(username, "Player ") {
                        ...
                        createdat := strings.Split(text, `"Created":`)[1]                                                                                             createdat = strings.Split(createdat, ",")[0]
                        country := strings.Split(text, `"Country":`)[1]                        country = strings.Split(country, ",")[0]
                        region := strings.Split(text, `"Region:"`)[1]
                        region = strings.Split(region, ",")[0]                                 jso, _ := json.MarshalIndent(fjson(text), "", "  ")
...
func req(url string) {
        ...
        jsonv, _ := json.Marshal(js)                                           re, _ := http.NewRequest("POST", url, bytes.NewBuffer(jsonv))
